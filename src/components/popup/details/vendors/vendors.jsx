...
export default class Vendors extends Component {
	...
	renderOptoutDescription(optOutDescription, textLinkColor) {
		...
		return (
			<p>
				Depending on the type of data they collect, use, and process and other
				factors including privacy by design, certain partners rely on your
				consent while others require you to opt-out. For information on each
				vendor and to exercise your choices, see below. Or to opt-out, visit the{' '}
				<a
	...
	render(props, state) {
		...
		const {textColor, textLightColor, textLinkColor, primaryColor} = theme;
			...
			<div class={style.vendors}>
				<div class={style.header}>
					<div class={detailsStyle.title} style={{color: textColor}}>
						<PurposesLabel localizeKey={`purpose${selectedPurposeId}.title`}>
							{name}
						</PurposesLabel>
					</div>
				</div>
				<div class={detailsStyle.description} style={{color: textLightColor}}>
					<p>
						<PurposesLabel
							localizeKey={`purpose${selectedPurposeId}.description`}
						>
							{description}
						</PurposesLabel>
					</p>
									...
									<tr key={id} class={index % 2 === 0 ? style.even : ''}>
										<td class={style.vendorItem}>
											...
											{showFeaturesId === id && (
												<div
													class={[
														index % 2 === 0 ? style.even : '',
														style.vendorFeatures
													].join(' ')}
												>
													<ul>
														{featureIds.map(featureId =>
															this.renderFeature(featureId)
														)}
													</ul>
												</div>
											)}
